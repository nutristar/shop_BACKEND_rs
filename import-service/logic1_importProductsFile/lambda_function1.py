...
def handler1(event, context):
    ...
    if name and re.match(r'^[\w,\s-]+\.[Cc][Ss][Vv]$', name):
        try:
            ...
            # Генерация подписанного POST запроса для загрузки файла
            response = s3_client.generate_presigned_post(
                'lunaxxximportservicebacket',
                f'uploaded/{name}',
                ExpiresIn=300
            )
            ...
            headers = {
                ...
                "Access-Control-Allow-Methods": "OPTIONS, POST",  # Указывает разрешенные методы
                "Access-Control-Allow-Headers": "Content-Type"  # Указывает разрешенные заголовки
            }

            # Возвращаем подписанный POST запрос и поля в теле ответа
            return {
                'statusCode': 200,
                'headers': headers,
